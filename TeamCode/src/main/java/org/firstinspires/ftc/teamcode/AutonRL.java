...
@Autonomous(name="Auton Red Left (RL)")
public class AutonRL extends LinearOpMode{

    //stuff to change

    boolean park = true;
    boolean farSide = true;
    ...
    public void placePurplePixel(boolean isRight, boolean isLeft, boolean isMiddle, OPMove12 karel){
        ...
        if(isMiddle){
            karel.forwards(0.5, 900);
            karel.stop(1000);
            karel.turnLeft(0.5, 520);
            karel.stop(1500);
            //in position for pixel placement
            karel.rackDown();
            karel.stop(1500);
            karel.rackUp();
            karel.left(0.5, 300);
            //after dropping pixel we back up towards wall to go around pixel
            karel.stop(1000);
            karel.forwards(0.5, 500);
            karel.stop(500);
            //move over to the right before complete turn *reduce this right strafe to reach correct parking area*
            karel.right(0.5, 1400);
            karel.stop(1000);
            //turn right is 180 *adjust turn to be preciser*
            karel.backwards(0.5, 400);
